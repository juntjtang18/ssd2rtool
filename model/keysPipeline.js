...
const EXCLUDED_EXTRA_KEYS = new Set(["UKEY", "TKEY", "HKEY", "DKEY", "KEYSET"]);
...
function computeTcDropPieces({ schedule, tcDropTable, keyRuns, phaseData, predictableMinProb }) {
  if (!tcDropTable || !keyRuns || !phaseData) {
    return { predictablePieces: {}, lotteryPieces: {}, vexPlusLambda: 0 };
  }

  const runs = {
    countess: Number(schedule?.runs?.Rc ?? 0),
    summoner: Number(schedule?.runs?.Rs ?? 0),
    nihl: Number(schedule?.runs?.Rn ?? 0),
  };

  const predictablePieces = {};
  const lotteryPieces = {};
  let vexPlusLambda = 0;

  const bosses = [
    { id: "countess", runCfg: keyRuns.countess, runs: runs.countess },
    { id: "summoner", runCfg: keyRuns.summoner, runs: runs.summoner },
    { id: "nihl", runCfg: keyRuns.nihl, runs: runs.nihl },
  ];

  for (const b of bosses) {
    if (!b.runCfg || !(b.runs > 0)) continue;

    // Key runs are tcSet-only (base.useMapTcCounts=false). Baseline map counts not required.
    const runTcCounts = deriveRunTcCounts({}, b.runCfg);

    const m = computeMapEvModel({
      runTcCounts,
      tcDropTable,
      phaseData,
      predictableMinProb,
    ...
    for (const [item, perRun] of Object.entries(m?.predictable?.expectedDrops ?? {})) {
      ...
      if (!k || EXCLUDED_EXTRA_KEYS.has(k)) continue;
      predictablePieces[k] = Number(predictablePieces[k] ?? 0) + (Number(perRun) * b.runs);
    }

    for (const [item, perRun] of Object.entries(m?.lottery?.expectedDrops ?? {})) {
      const k = String(item ?? "").trim().toUpperCase();
      if (!k || EXCLUDED_EXTRA_KEYS.has(k)) continue;
      lotteryPieces[k] = Number(lotteryPieces[k] ?? 0) + (Number(perRun) * b.runs);
    }

    vexPlusLambda += Number(m?.lottery?.vexPlusExpected ?? 0) * b.runs;
  }

  return { predictablePieces, lotteryPieces, vexPlusLambda };
}

function vexPlusProbFromLambda(lambda) {
  const x = Number(lambda ?? 0);
  if (!(x > 0)) return 0;
  return 1 - Math.exp(-x);
}
...
function practicalFromSchedule({ schedule, extras, priceTable, phase, tcDropTable, keyRuns, phaseData, predictableMinProb }) {
  ...
  const { predictablePieces, lotteryPieces, vexPlusLambda } = computeTcDropPieces({
    schedule,
    tcDropTable,
    keyRuns,
    phaseData,
  ...
  const extraRows = [];
  if (hasTc) {
    for (const [name, pieces] of Object.entries(predictablePieces ?? {})) {
      const piecesInt = Math.floor(Number(pieces ?? 0)); // trim off any drops smaller than 1
  ...
  if (hasTc) {
    // lottery pieces are EV (no flooring). Value is EV too.
    for (const [name, piecesEv] of Object.entries(lotteryPieces ?? {})) {
      const q = getRuneQuote(priceTable, phase, name);
      if (!q || !(q.O > 0) || !(q.N > 0)) {
        bonusRows.push({ name, piecesInt: 0, O: 0, orders: 0, ist: 0, N: 0, piecesEv: Number(piecesEv ?? 0), istEv: 0 });
        continue;
      }
      const istEv = (Number(piecesEv ?? 0) / q.O) * q.N;
      bonusPossibleIst += istEv;
      bonusRows.push({ name, piecesEv: Number(piecesEv ?? 0), istEv, O: q.O, N: q.N });
    }

    vexPlusProb = vexPlusProbFromLambda(vexPlusLambda);
  } else {
    for (const e of bonus) {
      ...
      const orders = Math.floor(piecesInt / q.O);
      const possible = orders * q.N;
      if (orders > 0) bonusPossibleIst += possible;

      bonusRows.push({ name, piecesInt, O: q.O, orders, ist: possible, N: q.N });
    }
  }
